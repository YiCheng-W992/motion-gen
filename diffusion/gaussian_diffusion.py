...
from diffusion.dpm_solver_pytorch import NoiseScheduleVP, DPM_Solver, expand_dims
from diffusion.spacing import build_spaced_timesteps, logsnr_from_alphas_cumprod
...
class ModelVarType(enum.Enum):
    ...
    LEARNED_RANGE = enum.auto() # how much randomness to use between small and large


class LossType(enum.Enum):
    # MSE 是 KL 的近似优化，而 KL 是理论最严格的优化
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        lambda_rcxyz=0.,        # 约束重建的局部关节 xyz
        lambda_vel=0.,          # 速度 loss
        lambda_pose=1.,
        lambda_orient=1.,
        lambda_loc=1.,
        data_rep='rot6d',
        lambda_root_vel=0.,     # root 整体速度 loss
        lambda_vel_rcxyz=0.,
        lambda_fc=0.,           #  foot contact loss
        lambda_target_loc=0.,
        **kargs,
    ):
        ...
        # alphas_cumprod_prev = [1.0, alpha_bar_0, alpha_bar_1, ..., alpha_bar_{T-2}]
        alphas = 1.0 - betas
        ...
        # predict ε → estimate x0 → compute μt → sample x(t-1)   x_{t-1} = μ_t + σ_t * z
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t):
        ...
        # mu = coef1[t] * x_start + coef2[t] * x_t
        posterior_mean = (
    ...
    def p_mean_variance(
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            if self.model_var_type == ModelVarType.LEARNED:     # 模型直接输出 log_variance
                ...
                model_log_variance = frac * max_log + (1 - frac) * min_log           # 在小/大方差之间插值
                model_variance = th.exp(model_log_variance)
        else:
            model_variance, model_log_variance = {
                # for fixedlarge, we set the initial (log-)variance like so
                # to get a better decoder log likelihood.
                ModelVarType.FIXED_LARGE: (   # 偏向原始前向噪声，随机性更大
                    np.append(self.posterior_variance[1], self.betas[1:]),
                    np.log(np.append(self.posterior_variance[1], self.betas[1:])),
                ),
                ModelVarType.FIXED_SMALL: (  # 采样更 deterministic
                    self.posterior_variance,
    ...
    def dpm_sample_loop(
        ...
        """
        Generate samples from the model using DPM-Solver.
        This supports discrete-time trained models by mapping continuous time to discrete timesteps.
        """
        if device is None:
        ...
        if skip_timesteps and init_image is None:
            init_image = th.zeros_like(x)

        if t_start is None:
            if skip_timesteps:
                t_start = float(self.num_timesteps - skip_timesteps) / float(self.num_timesteps)
            else:
                t_start = 1.0
        if t_end is None:
            t_end = 1.0 / float(self.num_timesteps)

        ns = NoiseScheduleVP(
            schedule='discrete',
            betas=th.tensor(self.betas, dtype=th.float32)
        )

        if init_image is not None:
            t_start_tensor = th.tensor([t_start], device=device, dtype=th.float32)
            alpha_t = ns.marginal_alpha(t_start_tensor)
            sigma_t = ns.marginal_std(t_start_tensor)
            x = expand_dims(alpha_t, x.dim()) * init_image.to(device) + expand_dims(sigma_t, x.dim()) * x

        def process_xstart(x0):
            if denoised_fn is not None:
                x0 = denoised_fn(x0)
            if clip_denoised:
                x0 = x0.clamp(-1, 1)
        ...
        def model_fn(x_in, t_continuous):
            t_input = t_continuous.to(device=device, dtype=th.float32) * self.num_timesteps - 1
            model_output = model(x_in, t_input, **model_kwargs)
            if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
                model_output, _ = th.split(model_output, x_in.shape[1], dim=1)

            if self.model_mean_type == ModelMeanType.EPSILON:
                return model_output
            if self.model_mean_type == ModelMeanType.START_X:
                x0 = process_xstart(model_output)
            elif self.model_mean_type == ModelMeanType.PREVIOUS_X:
                x0 = process_xstart(self._predict_xstart_from_xprev(x_t=x_in, t=t_index, xprev=model_output))
            else:
                raise NotImplementedError(self.model_mean_type)

            alpha_t = ns.marginal_alpha(t_continuous)
            sigma_t = ns.marginal_std(t_continuous)
            return (x_in - expand_dims(alpha_t, x_in.dim()) * x0) / expand_dims(sigma_t, x_in.dim())
    ...
    def logsnr_sample_loop(
        ...
        """
        Generate samples using spaced DDPM/DDIM with logSNR timestep spacing.
        """
        if device is None:
            device = next(model.parameters()).device
        if model_kwargs is None:
        ...
        if 'y' in model_kwargs and 'text' in model_kwargs['y']:
            # encoding once instead of each iteration saves lots of time
            model_kwargs['y']['text_embed'] = model.encode_text(model_kwargs['y']['text'])

        use_timesteps = build_spaced_timesteps(
            self.num_timesteps,
            steps,
            spacing="logsnr",
            betas=self.betas,
        ...
        eval_diffusion = SpacedDiffusion(
            ...
            lambda_rcxyz=self.lambda_rcxyz,
            lambda_vel=self.lambda_vel,
            lambda_pose=self.lambda_pose,
            lambda_orient=self.lambda_orient,
            lambda_loc=self.lambda_loc,
            data_rep=self.data_rep,
            lambda_root_vel=self.lambda_root_vel,
            lambda_vel_rcxyz=self.lambda_vel_rcxyz,
            lambda_fc=self.lambda_fc,
            lambda_target_loc=self.lambda_target_loc,
        )

        sample_loop = eval_diffusion.ddim_sample_loop if use_ddim else eval_diffusion.p_sample_loop
        return sample_loop(
            model,
            shape,
            noise=noise,
